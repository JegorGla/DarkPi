...
def init_classic_bruteforce_ui(parent_frame, go_back_callback=None):
    ...
    def run_attack():
        ...
        def attempt_attack(password):

            lock = threading.Lock()
            
            """Попытка атаки с данным паролем."""
            if stop_event.is_set():
                return False  # Если стоп-сигнал активирован, не продолжать попытки
            ...
            try:
                ...
                if response.status_code == 200 and "✅ Вход успешен!" in response.text:
                    with lock:
                        ...
                        text_box.yview("end")
                    stop_event.set()  # Устанавливаем событие для остановки всех потоков
                    return True
                else:
                    with lock:
        ...
        def run_in_threads():
            ...
            def attempt_attack_thread():
                """Попытка атаки с данным паролем из очереди."""
                while not password_queue.empty() and not stop_event.is_set():
                    password = password_queue.get()  # Берем пароль из очереди
                    success = attempt_attack(password)
                    password_queue.task_done()  # Отметим задачу как выполненную

                    if success:
                        break  # Если вход успешен, завершить
