...
def create_get_proxy_ui(parent_frame, go_back_callback=None):
    ...
    # Интервал обновления
    interval_label = ctk.CTkLabel(top_frame, text="Interval (min):")
    interval_label.pack(side="left", padx=(10, 5))
    
    interval_entry = ctk.CTkEntry(top_frame, width=0)
    ...
    def auto_update_loop():
        while not stop_auto_update.is_set():
            ...
            save_rechoise_proxy_interval(str(interval))  # 👈 СОХРАНЯЕМ как строку, как в settings.json
